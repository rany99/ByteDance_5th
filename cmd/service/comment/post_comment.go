...
type CommentResponse struct {
	CommentList *models2.Comment `json:"comment"`
}
...
func (p *PostCommentFlow) CheckUid() error {
	if err := models2.NewUserInfoDAO().IsUserInfoExist(p.uid); err != nil {
		return err
	}
	return nil
}
...
func (p *PostCommentFlow) CheckVid() error {
	if ok := models2.NewVideoDao().VideoAlreadyExist(p.vid); !ok {
...
func (p *PostCommentFlow) CreateComment() (*models2.Comment, error) {
	...
	if err := models2.NewCommentDao().CreateAndCntAddOne(&comment); err != nil {
		return nil, err
	}
	return &comment, nil
}
...
func (p *PostCommentFlow) DeleteComment() (*models2.Comment, error) {
	...
	//确认评论是否存在，并获得待删除评论指针
	if err := models2.NewCommentDao().QueryCommentById(p.commentId, &comment); err != nil {
		return nil, err
	}
	//删除评论
	if err := models2.NewCommentDao().DeleteAndCntSubOne(p.commentId, p.vid); err != nil {
		return nil, err
	}
	return &comment, nil
}
...
func FillComment(comment *models2.Comment) error {
	if comment == nil {
