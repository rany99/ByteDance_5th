...
func (v *VideoDao) QueryVideoListByUserId(id int64, list *[]*Video) error {
	if list == nil {
		return errors.New("QureyVideoListByUserId：list指针为空")
	}
...
// QueryVideoListByLastTimeAndLimit 根据lasttime返回之前的Limit个视频
func (v *VideoDao) QueryVideoListByLastTimeAndLimit(lasttime time.Time, limit int, list *[]*Video) error {
	if list == nil {
		return errors.New("QueryVideoListByLastTimeAndLimit：list指针为空")
	}
	return DB.Model(&Video{}).Where("created_at < ?", lasttime).
		Order("created_at ASC").Limit(limit).Select([]string{
		"id",
		"user_info_id",
		"play_url",
		"cover_url",
