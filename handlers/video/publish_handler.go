...
func PublishHandler(ctx *gin.Context) {
	...
	for _, file := range files {
		...
		savePath := filepath.Join("./videos", filename)
		...
		/*
			if err = util.SaveImageFromVideo(name, false); err != nil {
				PublishError(ctx, err.Error())
				continue
			}
		*/
		...
		snapShotPath := filepath.Join("./images", snapShotFileName)
		log.Println(snapShotPath)
		if err = cover.SnapShotFromVideo(savePath, snapShotPath, 1); err != nil {
			PublishError(ctx, err.Error())
			continue
		}
		if err = video.PostVideo(userId, filename, name+".mp4", title); err != nil {
